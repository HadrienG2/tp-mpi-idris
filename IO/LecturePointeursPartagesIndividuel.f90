...
! par une méthode utilisant des pointeurs partagés dans le fichier, en
...
program lecture_pointeurs_partages_individuel
    ...
    character(len=14), parameter :: base_nom_fichier_cible = "fichier_ptpain"
    ...
    integer :: file_mode, file_descriptor, current_reader_rank
    
    logical data_acquired
    ...
    file_mode = MPI_MODE_RDONLY + MPI_MODE_SEQUENTIAL
    ...
    ! Lecture via des pointeurs partagés, en mode individuel (donc avec synchro explicite)
    do current_reader_rank = 0, world_size-1
        if (world_rank == current_reader_rank) then
            call MPI_FILE_READ_SHARED(file_descriptor, &
                                      valeurs_lues(1), &
