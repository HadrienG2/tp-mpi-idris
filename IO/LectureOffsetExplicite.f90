...
! par une méthode utilisant des offsets explicites dans le fichier, en
! mode non collectif, et en extrait 121 valeurs
!
...
program lecture_offset_explicite
    ...
    integer(kind=MPI_OFFSET_KIND) :: offset_nul = 0
    
    character(len=*), parameter :: nom_fichier_source = "donnees.dat"
    character(len=14), parameter :: base_nom_fichier_cible = "fichier_offexp"
    ...
    ! Lecture par des addresses explicites (avec une vue appropriée)
    ...
    call MPI_FILE_SET_VIEW(file_descriptor, &
                           ...
                           MPI_INFO_NULL, &
                           return_code)
    call MPI_FILE_READ_AT(file_descriptor, &
                          offset_nul, &
                          valeurs_lues(1), &
